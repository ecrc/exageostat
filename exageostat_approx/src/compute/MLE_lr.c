 ...
 * */
	...
	MORSE_MLE_dcmg_Tile_Async(MorseLower, MORSE_descC, msequence, mrequest, &data->l1, &data->l1, initial_theta, data->dm);
	VERBOSE(" Done.\n");

	//Copy Nrand to Z
	VERBOSE("LR: Generate Normal Random Distribution Vector Z (Synthetic Dataset Dense Generation Phase) .....");
	MORSE_MLE_zcpy_Tile_Async(MORSE_descZ, Nrand, msequence, mrequest);
	VERBOSE(" Done.\n");
 ...
 * */
{
                ...
                MORSE_MLE_zcpy_Tile_Async(data->hicma_descZ, streamdata, hsequence, hrequest);
...
//		MORSE_MLE_zcpy_Tile_Async(data->descZ, Nrand, data->sequence, &data->request[0]);
//		
//		if (data->verbose == 1 && MORSE_My_Mpi_Rank() == 0)
//                        fprintf(stderr, " Done.\n");
...
//                MORSE_MLE_zcpy_Tile_Async(data->descZ, streamdata, data->sequence, &data->request[0]);
...
double HICMA_MLE_Tile(unsigned n, const double * theta, double * grad, void * app_data) {
...
double HICMA_MLE_Tile_Async(unsigned n, const double * theta, double * grad, void * data) {
//! Maximum Likelihood Evaluation (MLE)
/*!  -- HICMA-async
 * Returns the loglikelihhod value for the given theta.
 * @param[in] n: unsigned variable used by NLOPT library.
...
void HICMA_MLE_Predict_Allocate(MLE_data *MORSE_data, int nZmiss, int nZobs, int lts, int p_grid, int q_grid, int mse_flag)
...
double HICMA_MLE_Predict_Tile(MLE_data *MORSE_data, double * theta, int nZmiss, int nZobs, double *Zobs, double *Zactual, double *Zmiss, int n, int lts)
//! //Predict missing values base on a set of given values and covariance matrix
/*!  -- HICMA-sync
 ...
 * */
{
	...
	//data->hicma_descC22rk	= hicma_descC22rk;
        ...
        //MORSE_MLE_zcpy_Tile_Async(MORSE_descZobs, Zobs, msequence, mrequest);
	...
	VERBOSE(" Done.\n");
        ...
        if( Zactual != NULL)
        {
                ...
                //MORSE_MLE_zcpy_Tile_Async(MORSE_descZactual, Zactual, msequence, mrequest);
	...
	//	printf ("%f - %f\n", data->lobs.x[i], data->lobs.y[i]);
        ...
        MORSE_MLE_dcmg_Tile_Async(MorseLower, MORSE_descC12, hsequence, hrequest,  &data->lmiss, &data->lobs, theta, data->dm);
        MORSE_Sequence_Wait(hsequence);
...
void HICMA_Call(MLE_data  *data, int ncores, int gpus, int lts,  int p_grid, int q_grid, int N, int nZobs, int nZmiss)
