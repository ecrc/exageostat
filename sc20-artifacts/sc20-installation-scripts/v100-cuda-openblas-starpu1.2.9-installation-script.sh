...
sed -i 's/MPI_Address/MPI_Get_address/g' mpi/examples/user_datatype/my_interface.c
        if [ "$CUDAVALUE" == "ON" ]; then
                if [ "$MPIVALUE" == "ON" ]; then
                        CFLAGS=-w ./configure --disable-starpufft --enable-cuda --disable-opencl --prefix=$PWD/starpu_install  --enable-blas-lib=none --disable-starpu-top --disable-starpufft --disable-build-doc --disable-starpufft-examples   --disable-fortran --with-perf-model-dir=$PWD/starpu_install --enable-maxcudadev=8
                else
                        CFLAGS=-w ./configure --disable-starpufft --enable-cuda --disable-opencl --prefix=$PWD/starpu_install  --enable-blas-lib=none --disable-starpu-top --disable-starpufft --disable-build-doc --disable-starpufft-examples --disable-mpi  --disable-fortran --with-perf-model-dir=$PWD/starpu_install --enable-maxcudadev=8
                fi
        else
                if [ "$MPIVALUE" == "ON" ]; then
