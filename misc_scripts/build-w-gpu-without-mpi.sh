...
module load magma/2.3.0-gcc-5.5.0-openblas-cuda-9.0
module load cmake/3.9.6
module load hwloc/1.11.8-gcc-5.5.0
#module load openmpi/3.0.0-gcc-5.5.0
module load fxt/0.3.7-gcc-5.5.0
 module load starpu/1.2.4-gcc-5.5.0-cuda-9.0-openblas-openmpi-3.0.0-fxt
...
cd build
CC=gcc cmake .. -DCMAKE_BUILD_TYPE=Debug -DCHAMELEON_USE_MPI=OFF -DCMAKE_INSTALL_PREFIX=$PWD/installdir -DBUILD_SHARED_LIBS=ON -DCHAMELEON_USE_CUDA=ON
make -j
make install
...
echo 'module load openblas/0.2.20-gcc-5.5.0' >> ~/.bashrc
echo 'module load cuda/9.0' >> ~/.bashrc
echo 'module load magma/2.3.0-gcc-5.5.0-openblas-cuda-9.0' >> ~/.bashrc
...
echo 'module load starpu/1.2.4-gcc-5.5.0-cuda-9.0-openblas-openmpi-3.0.0-fxt' >> ~/.bashrc
