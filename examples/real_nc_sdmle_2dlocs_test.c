...
//MORSE_Lapack_to_Tile( streamdata_u, N, data.hicma_descZ);             
MORSE_MLE_sdzcpy(&data, streamdata_u);
/*#if defined(EXAGEOSTAT_USE_HICMA)
  else if (strcmp (data.computation, "lr_approx") == 0)
  HICMA_MLE_zcpy(&data, streamdata_u);
#endif	
 */
//MORSE_Lapack_to_Tile( streamdata_u, N, data.descZ);
	...
	if (strcmp(data.actualZLocFPath,"") != 0)
{
	...
	//   locations_obs_zsort_inplace(nZmiss, locations, streamdata_u);
}
