 ...
 * @ingroup double
 *
 *  CHAM_dpotrf - Computes the Cholesky factorization of a symmetric positive definite
...
int CHAMELEON_sdpotrf(CHAM_enum uplo, int N,
    ...
    if (chamctxt == NULL) {
        chameleon_fatal_error("CHAM_diag_dpotrf", "CHAMELEON not initialized");
        return CHAMELEON_ERR_NOT_INITIALIZED;
    }
    ...
    EXAGEOSTAT_sdpotrf_Tile_Async(uplo, &descAt, &temp, diag_thick, sequence, &request);
...
int EXAGEOSTAT_sdpotrf_Tile(CHAM_enum uplo, CHAM_desc_t *A, CHAM_desc_t *temp, int diag_thick) {
    ...
    chamctxt = chameleon_context_self();
    if (chamctxt == NULL) {
        chameleon_fatal_error("CHAMELEON_dpotrf_Tile", "CHAMELEON not initialized");
        return CHAMELEON_ERR_NOT_INITIALIZED;
    }
    chameleon_sequence_create(chamctxt, &sequence);
    EXAGEOSTAT_sdpotrf_Tile_Async(uplo, A, temp, diag_thick, sequence, &request);
 ...
 *
 ******************************************************************************/
int EXAGEOSTAT_sdpotrf_Tile_Async(CHAM_enum uplo, CHAM_desc_t *A, CHAM_desc_t *temp, int diag_thick,
                                 RUNTIME_sequence_t *sequence, RUNTIME_request_t *request) {
    CHAM_context_t *chamctxt;
    chamctxt = chameleon_context_self();
    if (chamctxt == NULL) {
        chameleon_fatal_error("CHAMELEON_dpotrf_Tile_Async", "CHAMELEON not initialized");
        return CHAMELEON_ERR_NOT_INITIALIZED;
    }
    if (sequence == NULL) {
    ...
    chameleon_psdpotrf(uplo, A, diag_thick, sequence, request);
    return CHAMELEON_SUCCESS;
}